...
DEFAULT_AUGMENTATION_ADD_RATIOS = [0.5, 1.0, 2.0]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run UT-HAR augmentation_add_ratio ablation")
    ...
    parser.add_argument(
        "--augmentation-add-ratios",
        nargs="+",
        type=float,
        default=DEFAULT_AUGMENTATION_ADD_RATIOS,
    )
    parser.add_argument(
        "--epochs",
        type=int,
    ...
    parser.add_argument("--reuse-existing-f5-ratio-one", action=argparse.BooleanOptionalAction, default=True)
    parser.add_argument("--allow-train-missing-ratio-one", action="store_true")
...
def _resolve_output_csv_path(args: argparse.Namespace) -> Path:
    ...
    return (
        metrics_dir / "final_augmentation_ratio_ablation_smoke_test_results.csv"
        if args.smoke_test
        else metrics_dir / "final_augmentation_ratio_ablation_results.csv"
    )
...
def _make_plan_key(
    ...
    augmentation_add_ratio: object,
    seed: object,
) -> tuple[str, str, str, str, str]:
    return (
        str(model),
        _float_key(real_ratio),
        str(preprocessing),
        _float_key(augmentation_add_ratio),
        str(int(seed)),
    )


def _row_to_ablation_schema(row: dict[str, object], source: str) -> dict[str, object]:
    return {
        "experiment": "augmentation_ratio_ablation",
        "model": row.get("model", ""),
        "real_ratio": row.get("real_ratio", ""),
        "augmentation_mode": row.get("augmentation_mode", "offline_append"),
        "augmentation_add_ratio": row.get("augmentation_add_ratio", ""),
        "preprocessing": row.get("preprocessing", ""),
        "seed": row.get("seed", ""),
        "training_mode": row.get("training_mode", "controlled_generalization"),
        "requested_epochs": row.get("requested_epochs", ""),
        "actual_epochs_ran": row.get("actual_epochs_ran", ""),
        "best_epoch": row.get("best_epoch", ""),
        "full_train_size": row.get("full_train_size", ""),
        "selected_real_train_size": row.get(
            "selected_real_train_size",
            row.get("selected_train_size", ""),
        ),
        "synthetic_train_size": row.get("synthetic_train_size", ""),
        "effective_train_size": row.get(
            "effective_train_size",
            row.get("selected_train_size", ""),
        ),
        "val_size": row.get("val_size", ""),
        "test_size": row.get("test_size", ""),
        "augmentation_config": row.get("augmentation_config", ""),
        "val_accuracy": row.get("val_accuracy", ""),
        "val_macro_f1": row.get("val_macro_f1", ""),
        "test_accuracy": row.get("test_accuracy", ""),
        "test_macro_f1": row.get("test_macro_f1", ""),
        "no_aug_test_macro_f1": row.get("no_aug_test_macro_f1", ""),
        "no_aug_test_accuracy": row.get("no_aug_test_accuracy", ""),
        "augmentation_gain_macro_f1": row.get("augmentation_gain_macro_f1", ""),
        "augmentation_gain_accuracy": row.get("augmentation_gain_accuracy", ""),
        "augmentation_relative_gain_macro_f1": row.get("augmentation_relative_gain_macro_f1", ""),
        "augmentation_relative_gain_accuracy": row.get("augmentation_relative_gain_accuracy", ""),
        "checkpoint_path": row.get("checkpoint_path", ""),
        "selected_by": row.get("selected_by", "validation_macro_f1"),
        "source": source,
    }


def _load_existing_ablation_rows(csv_path: Path, overwrite: bool) -> dict[tuple[str, str, str, str, str], dict[str, object]]:
    if overwrite or not csv_path.exists():
        return {}
    frame = pd.read_csv(csv_path)
    rows: dict[tuple[str, str, str, str, str], dict[str, object]] = {}
    for row in frame.to_dict(orient="records"):
        row.setdefault("source", "existing_ablation")
        key = _make_plan_key(
            row["model"],
            row["real_ratio"],
            row["preprocessing"],
            row["augmentation_add_ratio"],
            row["seed"],
        )
        rows[key] = _row_to_ablation_schema(row, source="existing_ablation")
    return rows


def _load_reusable_ratio_one_rows(
    csv_path: Path,
    ...
    if not csv_path.exists():
        return {}
    frame = pd.read_csv(csv_path)
    for column in ["real_ratio", "augmentation_add_ratio"]:
        if column in frame.columns:
            frame[column] = pd.to_numeric(frame[column], errors="coerce")

    filtered = frame[
        (frame["augmentation_mode"].astype(str) == "offline_append")
        & (frame["augmentation_add_ratio"] == 1.0)
        & (frame["preprocessing"].astype(str) == preprocessing)
        & (frame["model"].astype(str).isin(normalized_models))
        & (frame["real_ratio"].isin(real_ratios))
    ].copy()
    if "seed" in filtered.columns:
        filtered = filtered[pd.to_numeric(filtered["seed"], errors="coerce") == int(seed)]
    ...
    for row in filtered.to_dict(orient="records"):
        ...
        rows[key] = _row_to_ablation_schema(row, source="reused_final_f5")
    return rows
...
def _resolve_smoke_test(args: argparse.Namespace) -> None:
    ...
    if args.augmentation_add_ratios == DEFAULT_AUGMENTATION_ADD_RATIOS:
        args.augmentation_add_ratios = [1.0]
...
def _estimate_run_metadata(
    ...
    for model_name in normalized_models:
        for real_ratio in args.real_ratios:
            for augmentation_add_ratio in args.augmentation_add_ratios:
...
def _build_plan(
    ...
    reusable_ratio_one_rows: dict[tuple[str, str, str, str, str], dict[str, object]],
) -> list[dict[str, object]]:
    ...
    for model_name in normalized_models:
        for real_ratio in args.real_ratios:
            for augmentation_add_ratio in args.augmentation_add_ratios:
                ...
                estimate = estimate_map[key]
                if key in existing_rows:
                    planned_action = "skip_existing_ablation"
                elif (
                    args.reuse_existing_f5_ratio_one
                    and float(augmentation_add_ratio) == 1.0
                    and key in reusable_ratio_one_rows
                ):
                    planned_action = "reuse_final_f5"
                elif (
                    args.reuse_existing_f5_ratio_one
                    and float(augmentation_add_ratio) == 1.0
                    and not args.allow_train_missing_ratio_one
                ):
                    raise ValueError(
                        "Requested augmentation_add_ratio=1.0 but reusable rows are missing in "
                        "results/metrics/final_augmentation_results.csv. "
                        "Use --allow-train-missing-ratio-one to train missing ratio-one rows."
                    )
                else:
                    planned_action = "train"
                ...
                plan.append(
                    {
                        ...
                        "real_ratio": real_ratio,
                        "augmentation_add_ratio": augmentation_add_ratio,
                        "selected_real_train_size": estimate["selected_real_train_size"],
                        "synthetic_train_size": estimate["synthetic_train_size"],
                        "effective_train_size": estimate["effective_train_size"],
                        "val_size": estimate["val_size"],
                        "test_size": estimate["test_size"],
...
def _attach_augmentation_gains(
    ...
    if missing_mask.any():
        ...
        raise ValueError(
            "Missing same-real_ratio F4 baseline rows for augmentation ablation comparison: "
...
def main() -> None:
    ...
    if any(add_ratio < 0 for add_ratio in args.augmentation_add_ratios):
        raise ValueError("All augmentation_add_ratio values must be non-negative.")

    augmentation_config = resolve_augmentation_config(DEFAULT_AUGMENTATION_CONFIG)
    output_csv_path = _resolve_output_csv_path(args)
    existing_rows = _load_existing_ablation_rows(output_csv_path, overwrite=args.overwrite)
    reusable_ratio_one_rows = (
        _load_reusable_ratio_one_rows(
            PROJECT_ROOT / "results" / "metrics" / "final_augmentation_results.csv",
            normalized_models,
            args.real_ratios,
            args.preprocessing,
            args.seed,
        )
        if args.reuse_existing_f5_ratio_one
        else {}
    )
    resolved_config = {
        "experiment": "augmentation_ratio_ablation",
        ...
        "augmentation_add_ratios": args.augmentation_add_ratios,
        "reuse_existing_f5_ratio_one": args.reuse_existing_f5_ratio_one,
        "allow_train_missing_ratio_one": args.allow_train_missing_ratio_one,
        "epochs": args.epochs,
        "seed": args.seed,
        "batch_size": args.batch_size,
        "preprocessing": args.preprocessing,
    ...
    if not args.overwrite:
        rows_by_key.update(existing_rows)
    reused_keys: list[tuple[str, str, str, str, str]] = []
    skipped_keys: list[tuple[str, str, str, str, str]] = []

    for planned_row in plan:
        key = planned_row["key"]
        model_name = str(planned_row["model"])
        real_ratio = float(planned_row["real_ratio"])
        augmentation_add_ratio = float(planned_row["augmentation_add_ratio"])
        planned_action = str(planned_row["planned_action"])

        if planned_action == "skip_existing_ablation":
            skipped_keys.append(key)
            continue
        if planned_action == "reuse_final_f5":
            rows_by_key[key] = reusable_ratio_one_rows[key]
            reused_keys.append(key)
            continue

        if planned_action == "train":
                print(
                    "\nRunning model={model} real_ratio={ratio} augmentation_add_ratio={add_ratio}".format(
                        model=model_name,
                        ratio=real_ratio,
                        add_ratio=augmentation_add_ratio,
                    )
                )
                set_seed(args.seed)
                ...
                add_ratio_token = str(augmentation_add_ratio).replace(".", "p")
                ratio_token = str(real_ratio).replace(".", "p")
                ...
                checkpoint_path = (
                    ...
                    / "checkpoints"
                    / (
                        f"final_augmentation_ablation_{checkpoint_suffix}_"
                        f"{model_name.replace('+', '_').lower()}_{ratio_token}_add{add_ratio_token}_best.pt"
                    )
                )
                ...
                try:
                    ...
                    if torch.cuda.is_available() and "out of memory" in str(exc).lower():
                        ...
                        raise RuntimeError(
                            ...
                            f"and augmentation_add_ratio={augmentation_add_ratio}. Retry with --batch-size 32."
                        ) from exc
                    raise
    ...
    if reused_keys:
        print("Reusing augmentation_add_ratio=1.0 rows from final_augmentation_results.csv")
        for key in reused_keys:
            print(f"- reused {key[0]} real_ratio={key[1]} add_ratio={key[3]}")
    if skipped_keys:
        print("Skipping existing ablation rows:")
        for key in skipped_keys:
            print(f"- skipped {key[0]} real_ratio={key[1]} add_ratio={key[3]}")
    ...
    print(f"Saved augmentation ratio ablation results to: {output_csv_path}")
    ...
    if not args.disable_regenerate_figures:
        try:
            ...
            print(f"Warning: figure regeneration failed after augmentation ablation run: {exc}")
